...
def rekey_user(*, ctx: Context, user: AuthenticatedUser, old_password: str, new_password: str) -> bytes:
    r = ctx.conn.execute(
        """
        SELECT kdf_salt, pw_check_nonce, pw_check_ciphertext
        FROM users
        WHERE id = ?
        """,
        (user.user_id,),
    ...
    old_key = crypto.derive_key(old_password, salt=r["kdf_salt"])
    blob = crypto.EncryptedBlob(nonce=r["pw_check_nonce"], ciphertext=r["pw_check_ciphertext"])
    try:
        check = crypto.decrypt(blob, key=old_key)
    except Exception as e:
    ...
    try:
        ...
        ctx.conn.execute(
            "UPDATE users SET kdf_salt = ?, pw_check_nonce = ?, pw_check_ciphertext = ?, updated_at = ? WHERE id = ?",
            (new_salt, new_check.nonce, new_check.ciphertext, now, user.user_id),
        )
        for row in rows:
            plaintext = crypto.decrypt(row.encrypted, key=old_key)
            enc = crypto.encrypt(plaintext, key=new_key)
            ctx.conn.execute(
                """
                UPDATE entries
                SET content_nonce = ?, content_ciphertext = ?, updated_at = ?
                WHERE user_id = ? AND id = ?
                """,
                (enc.nonce, enc.ciphertext, now, user.user_id, row.id),
            )
        ctx.conn.commit()
    except Exception:
        ctx.conn.rollback()
